...
from bots.main import SekonomBot
from bots.sub.dollarubl import DollarublSubBot
...
from models.dollarubl import Currency, Request
import logging
    ...
    def wrapper():
        ...
        for currency, item in data.get('data', {}).items():
            ...
            try:
                ...
                currency_object = Currency.objects.get(code=currency, direct=Currency.DIRECT_SALE)
            ...
            try:
                ...
                currency_object = Currency.objects.get(code=currency, direct=Currency.DIRECT_BUY)
                logger.info('Success find')
                if currency_object.use_api:
                    logger.info('Update')
                    currency_object.value = value
...
def send_fails(requests=None):
    ...
    for request in requests:
        ...
        yield DollarublSubBot(
            chat_id=request.user.out_id,
            parent_bot=SekonomBot()
        ).send(Text.format('CL_FAIL_RESERV_TIMEOUT'))
    ...
    for request in requests:
        if request.user.get_id() not in users:
            ...
            yield DollarublSubBot(
                chat_id=request.user.out_id,
                parent_bot=SekonomBot()
            ).send(Text.format('CL_REQUEST_RATING'), reply_markup=keyboard)
